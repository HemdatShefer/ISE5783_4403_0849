...
class GeometriesTest {
    ...
    void testFindIntsersections() {
        ...
        result = geos.findIntsersections(new Ray(new Point(1, 0, 0), new Vector(7, 3, 0)));
        assertNotNull(result, "It is empty!");
        assertEquals(3, result.size(), "Bad intersects");

        // =============== Boundary Values Tests ==================
        // TC11: Empty collection
        result = new Geometries().findIntsersections(new Ray(new Point(1, 0, 0), new Vector(1, 0, 0)));
        assertNull(result, "It is not empty!");

        // TC12: None geo intersect
        result = geos.findIntsersections(new Ray(new Point(1, 0, 0), new Vector(1, 3, 0)));
        assertNull(result, "Bad intersects");

        // TC13: Single geo intersect
        result = geos.findIntsersections(new Ray(new Point(1, 0, 0), new Vector(4, 3, 0)));
        assertNotNull(result, "It is empty!");
        assertEquals(1, result.size(), "Bad intersects");

        // TC14: All geo intersect
        result = geos.findIntsersections(new Ray(new Point(1, 0, 0), new Vector(7, 1, 0)));
        assertNotNull(result, "It is empty!");
